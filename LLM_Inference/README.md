...
本文旨在加速 LLM 解码while考虑内核的 dynamically changing characteristics, 提出PAPI（基于PIM的并行解码）: a PIM-enabled heterogeneous architecture that exploits dynamic scheduling of compute-bound or memory-bound kernels to suitable hardware units. 包含两个关键机制：
...
#### 2.2.1.Batching
并发处理多个输入序列allows a single decoding step to generate multiple tokens from different, user requests. $\Rightarrow$ <u>请求级并行/RLP</u>: 并行执行的请求数量。

batch size = 一次同时处理多少个请求

mixed continuous batching [16, 17]: 系统会根据可用内存和计算资源as well as the number of incoming requests，在运行时动态调整批次中的请求数量。
- 允许在无需等待批次中所有请求完成的情况下，将新请求添加到当前批次中，从而优化资源利用率并提高整体吞吐量。

#### 2.2.2.Speculative Decoding
[18, 19] 
- 串行草稿解码 serial draft decoding: a small draft model efficiently predicts the next 2-10 tokens
- 并行推测解码 parallel speculative decoding: 将这些预测的词元与大型原始 LLM 同时验证，从而实现后续词元的并行处理。

speculative length = 一次“猜”几个 token

实现了 LLM 推理中的<u>词元级并行性/TLP</u>: 在一次解码迭代中同时解码的词元数量。

## 3.动机
### 3.1.LLM推断分析
我们通过评估全连接（FC）和多头注意力核的算术强度来分析LLM推理的计算和内存需求。
- 变量：改变批处理大小（启用批处理时）和推测长度（启用推测解码时）
- 模型：OPT-30B model
  - using a high-end NVIDIA A100 GPU [29] with 312 TFLOPS peak computation performance and 1935 GB/s peak memory bandwidth
- 结论：
  <img src="image-1.png" width="50%">
  - batch size 增大, 权重 W 被更多 token / request 复用, 每 Byte 数据“干的活更多”，AI_FC 就上升
  - 注意力核的运算强度不会随批处理大小的增加而改变，因为注意力核在批处理过程中不存在数据重用

$\textnormal{Arithmetic Intensity} = \frac{总计算量 FLOPs}{总数据搬运量 Bytes}$
$\textnormal{Performance} = \frac{FLOPs}{\textnormal{measured runtime}} = \min (\textnormal{峰值算力P}_{peak}, \textnormal{算术强度 AI}\times \textnormal{峰值带宽BW}_{peak})$

### 3.2.LLM推理中不同的并行化级别
**Initial Request-Level Parallelism (Initial RLP)**: 批量执行开始时的 RLP。
在实际的 LLM 服务场景中，初始 RLP 可能存在显著差异，导致批处理大小变化很大。原因:
- Service Level Objective (SLO) Limits：提高 RLP 可以提高吞吐量，但会增加inference latency per request [33]. 在线服务场景下，低用户延迟 SLO 决定了batch size也很低。
  > e.g. DGX A100 计算系统 [34] 配备 1,280 GB 内存，每批最多可支持 854 个请求，30 毫秒的 SLO 要求初始 RLP 设置为低至 22 [23]。
- Memory Capacity Limits：特别是对于 KV cache storage。一个拥有 640 GB HBM 的计算系统可以容纳 282 个输入输出长度为 128 的请求，但只能容纳 18 个输入输出长度为 2048 的请求 [23]。在后一种情况下，批次大小需要更小，因为 longer sequences need more memory capacity for KV cache for multi-head attention.
- Dynamic Batching：动态批处理 [35] starts processing a batch once the batch is full or exceeds a time limit。因此，当请求不频繁时，采用动态批处理的LLM服务系统可能会以不同的批次大小开始处理，从而产生不同的RLP值。

**Runtime Request-Level Parallelism (Runtime RLP)**：在执行一批请求期间的 RLP。取决于所使用的批处理机制，可以是静态批处理或混合连续批处理 [17]。
- Traditional LLM serving systems [36, 35] use static batching with batch-level scheduling. 只有当前批次中的所有请求都完成后才会处理新的请求。由于每个请求的输出长度都不同，运行时 RLP 会动态变化。
- Mixed continuous batching [16, 17]：在 LLM 服务系统执行当前批次请求时，可以向其添加新的请求进行处理。具体 RLP 调整取决于何时以及向每个批次添加多少请求。

**Token-Level Parallelism (TLP)**：[28] 在每次解码迭代期间调整推测长度。[38] 批处理和推测解码需要协同优化，以提高 LLM 推理的 GPU 利用率, e.g. 当批次大小较小时，可以增加推测长度以最大限度地利用资源。

#### 作者总结
批处理大小和推测长度在运行时会发生显著变化 (并行化程度的变化)，FC 和注意力机制的运算强度也会动态变化。所以，当在以计算为中心的系统（例如GPU）中执行时，全连接和注意力机制可能成为计算密集型或内存密集型机制。
- LLM 服务需要异构计算架构，具有先进的计算单元，可提供不同的计算吞吐量和内存带宽能力，以满足内核不同的算术强度。
- LLM 服务需要动态调度方法将 FC 内核映射到不同的计算单元，因为 FC 内核在运行时可能会在计算密集型和内存密集型之间切换。

### 3.3.现有内存处理架构在LLM推理方面的局限性
